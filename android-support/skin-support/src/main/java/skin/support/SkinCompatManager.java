...
public class SkinCompatManager extends SkinObservable {
    ...
    public AsyncTask loadSkin() {
        ...
        Log.d(TAG, "loadSkin:: skin:" + skin + ",strategy:" + strategy);
        if (TextUtils.isEmpty(skin) || strategy == SKIN_LOADER_STRATEGY_NONE) {
            Log.d(TAG, "loadSkin:: xxxx");
            return null;
        }
    ...
    public AsyncTask loadSkin(String skinName, SkinLoaderListener listener, int strategy) {
        ...
        Log.w(TAG, Log.getStackTraceString(new IllegalArgumentException("skinName:" + skinName + ",strategy:" + strategy)));
    ...
    private class SkinLoadTask extends AsyncTask<String, Void, String> {
        ...
        protected String doInBackground(String... params) {
            synchronized (mLock) {
                Log.d(TAG, "doInBackground:: 1,mLoading:" + mLoading);
                while (mLoading) {
                    try {
                ...
                mLoading = true;
                Log.d(TAG, "doInBackground:: 4,mLoading:" + mLoading + ",params.length=" + params.length + ",params[0]:" + params[0]);
            }
            try {
                if (params.length == 1) {
                    String skinName = mStrategy.loadSkinInBackground(mAppContext, params[0]);
                    Log.d(TAG, "doInBackground:: skinName:" + skinName);
        ...
        protected void onPostExecute(String skinName) {
            synchronized (mLock) {
                ...
                mLoading = false;
                mLock.notifyAll();
                Log.d(TAG, "onPostExecute:: notifyAll ...");
            }
        }
    }
